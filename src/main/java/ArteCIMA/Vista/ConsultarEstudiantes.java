...
import ArteCIMA.DAO.ConsultarEstudiantesDAO;
...
public class ConsultarEstudiantes extends javax.swing.JFrame {
    ...
    private static final java.util.logging.Logger logger = java.util.logging.Logger.getLogger(ConsultarEstudiantes.class.getName());

    private ConsultarEstudiantesDAO dao;
    ...
    public ConsultarEstudiantes() {
        ...
        dao = new ConsultarEstudiantesDAO();
   ...
   private boolean validarCampos() {
        ...
        if (!correo.isEmpty() && (!correo.contains("@") || !correo.contains("."))) {
            JOptionPane.showMessageDialog(this, "Correo inválido. Formato esperado: ejemplo@dominio.com");
            return false;
        }
    ...
    private void initComponents() {
        ...
        jLabel2.setText("CONSULTA DE ESTUDIANTES");
        ...
        jPanel1Layout.setHorizontalGroup(
            ...
            .addGroup(jPanel1Layout.createSequentialGroup()
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        .addGap(59, 59, 59)
                        .addComponent(jPanel2, javax.swing.GroupLayout.PREFERRED_SIZE, 1219, javax.swing.GroupLayout.PREFERRED_SIZE)))
    ...
    private void btnEditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEditarActionPerformed
        ...
        int fila = tblEstudiantes.getSelectedRow();
        if (fila == -1) {
            JOptionPane.showMessageDialog(this, "Seleccione un estudiante en la tabla para editar.");
            return;
        }
        ...
        est.setIdEstudiante(idEst);
        est.setTipoDocumento(comboTipoDoc.getSelectedItem().toString());
        est.setNumDocumento(txtNumDoc.getText().trim());
        est.setNombreCompleto(txtNombre.getText().trim());
                
        est.setEdad(Integer.parseInt(txtEdad.getText().trim()));
        
        est.setTelefono(txtTelefono.getText().trim());
        est.setCorreo(txtCorreo.getText().trim());
        est.setDiscapacidad(comboDiscapacidad.getSelectedItem().toString());
                
        String tipoDisc = comboDiscapacidad.getSelectedItem().toString().equalsIgnoreCase("No") ? null : txtTipoDiscapacidad.getText().trim();
        est.setTipoDiscapacidad(tipoDisc); 

        est.setTipoBeneficio(comboBeneficio.getSelectedItem().toString());
        
        est.setIdGrupo(txtIdGrupo.getText().trim().isEmpty() ? null : Integer.parseInt(txtIdGrupo.getText().trim()));
        est.setIdBeca(txtIdBeca.getText().trim().isEmpty() ? null : Integer.parseInt(txtIdBeca.getText().trim()));
        est.setIdAcudiente(txtIdAcudiente.getText().trim().isEmpty() ? null : Integer.parseInt(txtIdAcudiente.getText().trim()));
        
        ConsultarEstudiantesDAO dao = new ConsultarEstudiantesDAO();
        boolean ok = dao.editarEstudiante(est);
        ...
        if (ok) {
            ...
            JOptionPane.showMessageDialog(this, "Error al actualizar el estudiante. Revisa la consola para más detalles (claves foráneas, etc.).");
        }
    ...
    if (fila == -1) {
        JOptionPane.showMessageDialog(this, "Seleccione un estudiante en la tabla para eliminar.");
        return;
    }
    ...
    ConsultarEstudiantesDAO dao = new ConsultarEstudiantesDAO();
    boolean eliminado = dao.eliminarEstudiante(idEst);
    ...
    if (criterio.isEmpty()) {
        JOptionPane.showMessageDialog(this, "Ingrese ID, Número de Documento o Nombre del estudiante para buscar.", "Información", JOptionPane.INFORMATION_MESSAGE);
        limpiarCampos(); 
        return;
    }
    
    // 2. Ejecutar la búsqueda en la base de datos
    // 'dao' debe estar inicializado (dao = new ConsultarEstudiantesDAO())
    Estudiante est = dao.buscarEstudiante(criterio); 
    ...
    if (est == null) {
        JOptionPane.showMessageDialog(this, "No se encontró el estudiante con el criterio ingresado.", "Resultado de Búsqueda", JOptionPane.WARNING_MESSAGE);
        return;
    }
    ...
    comboDiscapacidad.setSelectedItem(est.getDiscapacidad() == null || est.getDiscapacidad().isEmpty() ? "Selecciona" : est.getDiscapacidad());
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> new ConsultarEstudiantes().setVisible(true));
    }
